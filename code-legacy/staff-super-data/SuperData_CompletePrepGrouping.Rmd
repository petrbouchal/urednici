...
# add salary and inflation [deflator] data
hh <- AddEconIndicators(hh)
hh$realchange <- hh$perc_base/hh$Infl2003Base
```

```{r create markers}
# recreate split measure and variable markers
hh$promenna <- str_split_fixed(hh$variable,'_',2)[,1]
hh$udaj <- str_split_fixed(hh$variable,'_',2)[,2]

hh$zmena <- ifelse(hh$udaj %in% c('schvaleny','upraveny','skutecnost'),
                   FALSE, TRUE)
...
# create markers for groups which are additions of original groups
hh <- MarkSumGroups(hh)

# create vector of colours for chart, in order of groups
grpcols_ordered <- idcols[c(6,5,4,1,3,2)]

# create more user-friendly group labels, to be used in graphs
...
write.csv(hh,paste0('./data-output/', csvname), row.names=FALSE)
